...
def prediction(cust_dir,view=False):
    ...
    for ff in file:
        ...
        k+=1
    if view==False:
        df.to_csv('prediction_sheet.csv',index=False)
        thd=[0.3,0.4,0.5,0.6,0.7,0.8,0.9,0.95]
        result = pd.DataFrame(columns=['iou_threshold','Accracy','Recall','Precision','F_Measure','TP','FN','FP','TN'])
  
        for t in thd:
            acc,recall,precision,f_measure,tp,fn,fp,tn=test.cal_score("ground_truth_sheet.csv","prediction_sheet.csv",t)
            print("\n****** iou threshold:{} ***********\nConfusion Matrix\n{},{}\n{},{}\n\nAccuracy:{}, Recall:{}, Precision:{}, F_measure:{}\n*******************".format(t,tp,fn,fp,tn,round(acc,3),round(recall,3),round(precision,3),round(f_measure,3)))
            result=result.append({'iou_threshold':t,'Accracy':round(acc,3),'Recall':round(recall,3),'Precision':round(precision,3),'F_Measure':round(f_measure,3),'TP':tp,'FN':fn,'FP':fp,'TN':tn} , ignore_index=True)
